...
struct ProfileEditView: View {
    ...
    var body: some View {
        ...
        .onAppear {
            ...
            // Если настройки заполнены, автоматически загружаем модели
            if canLoadModels {
                loadModels()
            }
        }
    ...
    // ДОБАВЛЕНО: Новые вспомогательные методы
    private func updateCanLoadModels() {
    ...
    private func loadModels() {
        ...
        if let cachedData = UserDefaults.standard.data(forKey: cacheKey),
           ...
           !cachedModels.isEmpty {
            ...
            // Все еще делаем фоновую загрузку для обновления
            Task {
    ...
    private func loadModelsFromAPI(cacheKey: String) async {
        do {
            ...
            await MainActor.run {
                ...
                // Если текущая модель не найдена в списке, переключаемся на ручной ввод
                if !modelName.isEmpty && !models.contains(where: { $0.id == modelName }) {
                    useManualModel = true
                    manualModelName = modelName
                }
                
                // Если ручной ввод не активен и модель не выбрана, предлагаем первую доступную
                if !useManualModel && modelName.isEmpty && !models.isEmpty {
                    // Находим популярную модель или берем первую
            ...
            await MainActor.run {
                ...
                // При ошибке переключаемся на ручной ввод
                if !useManualModel {
                    useManualModel = true
                    manualModelName = modelName
                }
